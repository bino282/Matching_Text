...
class MATCH_LSTM_CNN():
    ...
    def build(self,conf):
        ...
        seq1_rep_rnn = LSTM(self.config['hidden_size'], return_sequences=True, dropout=self.config['dropout_rate'])(seq1_embed)
        seq2_rep_rnn = LSTM(self.config['hidden_size'], return_sequences=True, dropout=self.config['dropout_rate'])(seq2_embed)
        ...
        qi_aggreg, nc_cnn = self.aggregate(self.config['seq1_maxlen'], l2reg=conf['l2reg'], cnninit=conf['cnninit'], 
                                    cnnact=conf['cnnact'], input_dim=self.config['hidden_size'], inputs=seq1_rep_rnn, 
                                    cdim={1: 1/2, 2: 1/2, 3: 1/2, 4: 1/2, 5: 1/2}, pfx='aggre_q')
        ...
        si_aggreg, nc_cnn = self.aggregate(self.config['seq1_maxlen'], l2reg=conf['l2reg'], cnninit=conf['cnninit'], 
                                    cnnact=conf['cnnact'], input_dim= self.config['hidden_size'], inputs=si_foc, 
    ...
    def focus(self,N,seq_len, input_aggreg, input_seq, orig_seq, sdim, awidth, l2reg, pfx=''):
        ...
        attn = RepeatVector(self.config['hidden_size'])(attn)
